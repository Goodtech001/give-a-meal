...
function Cards({ data, numCards }: CardsProps) {
  ...
  return (
    ...
    >
      <div className=" gap-5 items-center justify-center mt-25 relative grid grid-cols-3 space-x-4">
        {data.slice(0, numCards).map((card, index) => (
          <motion.div
            key={index}
            className="relative border w-fit rounded-t-2xl border-gray-300 mb-5"
          ...
          >
            ...
            <div className="p-6">
              <div>
                <h1 className="font mt-2 text-left text-2xl font-semibold text-[#03324E] line-clamp">
                  {truncateText(card.title, 20)}
                </h1>
                <p className="font mt-2 text-[#00304CCC] ">{truncateText(card.description, 20)}</p>
              </div>
